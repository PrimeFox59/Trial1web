...
def build_drive_service_from_secrets():
    """Load credentials from Streamlit secrets (not from file)"""
    if "gdrive_service_account" not in st.secrets:
        st.error("Google Drive service account credentials not found in Streamlit secrets!")
        st.stop()
...
def page_gdrive():
    st.title("📂 Google Drive Integration (via Streamlit Secrets)")
    service, sa_email = build_drive_service_from_secrets()

    with st.expander("Instruksi"):
        st.markdown(
            """
            1. Pastikan service account sudah diisi di Streamlit secrets.
...
def main():
    ...
    # --- Google Drive Service Account Key Input ---
    if "gdrive_service_account" not in st.session_state:
        st.sidebar.markdown("**Google Drive Service Account Key**")
        gdrive_key_text = st.sidebar.text_area("Paste JSON Service Account Key di sini", value="", height=200)
        if st.sidebar.button("Simpan Key Google Drive"):
            try:
                st.session_state.gdrive_service_account = json.loads(gdrive_key_text)
                st.sidebar.success("Service Account Key berhasil disimpan!")
            except Exception as e:
                st.sidebar.error(f"Format JSON tidak valid: {e}")

    # Navigasi dan user logic ...existing code...
    if not user:
